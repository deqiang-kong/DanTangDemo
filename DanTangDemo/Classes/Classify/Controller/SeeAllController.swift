...
let seeAllcellID = "SeeAllTopicCell"

class SeeAllController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let nib = UINib(nibName: String(describing: SeeAllTopicCell.self), bundle: nil)
        tableView.register(nib, forCellReuseIdentifier: seeAllcellID)
        tableView.separatorStyle = .none
        tableView.rowHeight = 160
        /// 分类界面 顶部 专题合集
        NetworkTool.shareNetworkTool.loadCategoryCollection(limit: 20) { (collections) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: seeAllcellID) as! SeeAllTopicCell
