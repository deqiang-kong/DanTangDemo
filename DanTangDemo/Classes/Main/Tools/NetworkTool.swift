...
class NetworkTool: NSObject {
    ...
    /// 获取首页数据
    func loadHomeInfo(id: Int, finished:@escaping (_ homeItems: [HomeItem]) -> ()) {
            ...
            .responseJSON { (response) in
                ...
                if let value = response.result.value {
                    ...
                    if let items = data!["items"]?.arrayObject {
                        ...
                        for item in items {
                            let homeItem = HomeItem(dict: item as! [String: AnyObject])
    ...
    /// 获取首页顶部选择数据
    func loadHomeTopData(finished:@escaping (_ ym_channels: [Channel]) -> ()) {
            ...
            .responseJSON { (response) in
                ...
                if let value = response.result.value {
                    ...
                    if let channels = data!["channels"]?.arrayObject {
                        var ym_channels = [Channel]()
                        for channel in channels {
                            let ym_channel = Channel(dict: channel as! [String: AnyObject])
    ...
    /// 根据搜索条件进行搜索
    func loadSearchResult(keyword: String, sort: String, finished:@escaping (_ results: [SearchResult]) -> ()) {
        SVProgressHUD.show(withStatus: "正在加载...")
    ...
    /// 分类界面 顶部 专题合集
    func loadCategoryCollection(limit: Int, finished:@escaping (_ collections: [Collection]) -> ()) {
        SVProgressHUD.show(withStatus: "正在加载...")
    ...
    /// 顶部 专题合集 -> 专题列表
    func loadCollectionPosts(id: Int, finished:@escaping (_ posts: [CollectionPost]) -> ()) {
        SVProgressHUD.show(withStatus: "正在加载...")
    ...
    /// 底部 风格品类 -> 列表
    func loadStylesOrCategoryInfo(id: Int, finished:@escaping (_ items: [CollectionPost]) -> ()) {
        SVProgressHUD.show(withStatus: "正在加载...")
        let url = BASE_URL + "v1/channels/\(id)/items"
        let params = ["limit": 20,
                      "offset": 0]
